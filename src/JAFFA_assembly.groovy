...
find_fusions = segment { run_assembly + 
...
// The actual pipeline.
//if(readLayout=="single"){ fastqInputFormat="%.(gz|bam)" }

run {  
	run_check + [
		"%.gz" * [ make_dir + prepare_reads_from_fastq + find_fusions ],
		"%.bam" * [ make_dir + prepare_reads_from_bam + find_fusions ] 
	] + compile_all_results
}
