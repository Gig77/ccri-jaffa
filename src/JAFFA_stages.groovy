...
//exclude="NoSupport,PotentialRegularTranscript" //fusions marked with these classifications will be thrown away. Can be a comma seperated list. 
exclude="nothing"
...
prepare_reads_from_bam = {
	...
	from("*.bam") {
	     ...
	     produce(base+"_filtered_reads.fastq.gz",
		     base+"_leftover_reads.fastq.gz"){
             exec """
                 java -jar /usr/local/JAFFA-version-1.06/tools/picard-tools-1.114/SamToFastq.jar VALIDATION_STRINGENCY=SILENT INPUT=$input.bam FASTQ=/dev/stdout | $trimmomatic SE -threads $threads -phred$scores /dev/stdin $base/${base}_trim.fastq
                         LEADING:$minQScore TRAILING:$minQScore MINLEN:$minlen ;
              $bowtie2 $mapParams --very-fast
               	        --al-gz $output1 --un $base/temp_trans_unmap_reads.fastq
               		-p $threads -x $transFasta.prefix -U $base/${base}_trim.fastq
